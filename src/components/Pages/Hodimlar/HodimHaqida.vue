          ...
          <div class="col-md">
            <div class="row">
              <div class="col-md-4 mb-2">
                ...
                <input type="date" id="dan" class="form-control" v-model="dateFrom" />
              </div>
              <div class="col-md-4">
                <label for="gacha">Gacha</label>
                <input type="date" id="gacha" class="form-control" v-model="dateTo"/>
              </div>
              <div class="col-md-4" style="margin-top: 33px;">
                <div class="row">
                  <div class="col-md">
                    <button class="btn btn-block btn-outline-success" @click="sortByDate(dateFrom, dateTo)">
                      <span class="fa fa-search"></span>
                    </button>
                  </div>
                  <div class="col-md">
                    <button class="btn btn-block btn-outline-primary" @click="getData()">
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.isloading = true
      instance.get("this_user_expense/" + this.user_id)
      .then((response) => {
      ...
      this.isloading = true
      instance.get("this_user/" + this.user_id)
      .then((response) => {
      ...
      this.isloading = true
      instance.get("this_user_kpi_history/" + this.user_id)
      .then((response) => {
    ...
    getThisTrade(object) {
      this.thisTrade = []
      instance.get("this_trade/" + object.trade_id)
      .then((response) => {
        instance.get("this_product/empty/" + response.data.product_code)
        .then((res) => {
    ...
    sortByDate(from, to) {
      if (from !== "" && to !== "") {
        ...
        this.payHistory.forEach(element => {
          if (Number(element.time[8] + element.time[9]) >= from2 && Number(element.time[8] + element.time[9]) <= to2) {
            tarix.push(element)
          }
        });
